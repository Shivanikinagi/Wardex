 ...
 * deploy-continue.js
 *
 * Continues deployment using the already-deployed wardex contract.
 * Run this if deploy.js failed after Step 1 (wardex already deployed).
 *
 * Usage:
...
async function main() {
    console.log("═══════════════════════════════════════════════════════════");
    console.log("     🔒 wardex — Continuing Deployment (Base Sepolia)");
    console.log("═══════════════════════════════════════════════════════════\n");
    ...
    const wardexAddress = DARK_AGENT_ADDRESS;
    console.log("♻️  Reusing wardex:", wardexAddress);
    const wardex = await ethers.getContractFactory("wardex");
    const wardex = wardex.attach(wardexAddress);
    console.log();
    ...
    const capabilityCheck = await CapabilityCheck.deploy(wardexAddress);
    ...
    const verifier = await Verifier.deploy(wardexAddress);
    ...
    console.log(`   wardex_CONTRACT=${wardexAddress}`);
    ...
    console.log("\n🔗 View on BaseScan:");
    console.log(`   https://sepolia.basescan.org/address/${wardexAddress}`);
    ...
    const deployment = {
        ...
        attestationHash,
        explorerUrl: `https://sepolia.basescan.org/address/${wardexAddress}`,
    ...
    return { wardexAddress, capabilityCheckAddress, verifierAddress };
}
